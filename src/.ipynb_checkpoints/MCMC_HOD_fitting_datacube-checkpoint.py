...
sh_table = np.load('/cosma7/data/dp004/dc-armi2/HOD_mocks/halo_catalogues/Haloes_MG-Gadget_GR_z0.3_L768_ID_subhalotable_M200c_12.0.npy',allow_pickle=True)
...
rp = wp_cols[:,0]
wp_data = wp_cols[:,1]
wp_err = wp_cols[:,2]*3#np.sqrt((wp_cols[:,2]**2 + (0.10*wp_cols[:,1])**2))
...
if rank ==1:
    name_datacube = '/cosma7/data/dp004/dc-armi2/mcmc_runs/temps/datacube/datacube_test1.pkl'
...
PR = np.array([[12.5,14.5], #Prior ranges
             [12.5,14.5],
             [12.5,14.5],
             [0.0,0.9],
             [0.7,1.5]])  
...
def lnprob(theta,ydata=Y_data):
    ...
    lp = lnprior(theta) 
    if lp == 0:
        new_row = {'name_id':'%.5lf_%.5lf_%.5lf_%.5lf_%.5lf'%tuple(theta),'params':theta,'ngal':ymodel[0][0],'wp':ymodel[1][:,0]}
        new_row = comm.gather(new_row,root=1)
        if rank==1:
            print('I root (rank {}) gathered all objects {}'.format(rank,theta_job))
            for row in new_row:
                Mock_datacube.add_row_dc(row)
        return lp + lnlike(theta,ydata)
    else:
        return -np.inf

if rank == 0:
    filename = "/cosma7/data/dp004/dc-armi2/mcmc_runs/backends/mcmc_backend_stretch_lgprob_chain_"+str(nwalkers)+"wlk_"+str(burnin_it)+"burnin_"+str(prod_it)+"production_"+M+".h5"
